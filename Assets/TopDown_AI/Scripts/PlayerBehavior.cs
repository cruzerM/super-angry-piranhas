...
public class PlayerBehavior : MonoBehaviour {
	...
	public Transform hitTestPivot,gunPivot,gunPivot2;
	...
	//	float speedY = inputVertical > 0.1 ? Mathf.Clamp ((inputVertical * moveSpeed), moveSpeed / 2.0f, moveSpeed) : 0.0f;
		...
		switch (currentWeapon) {
			...
			case PlayerWeaponType.PISTOL:
				if (Input.GetMouseButtonDown (0) && attackTimer.IsFinished() && GameObject.Find("Flowchart").GetComponent<Block>().IsExecuting() == false) {
	...
	public void Attack(){
		switch (currentWeapon) {
			...
			GameObject bullet2=GameObject.Instantiate(proyectilePrefab, gunPivot2.position,gunPivot.rotation) as GameObject;
                GameObject.Find("Cartouch").GetComponent<AudioSource>().Play();
				bullet.transform.LookAt(mousePointer.transform);
				bullet.transform.Rotate(0,Random.Range(-7.5f,7.5f),0);
			bullet2.transform.LookAt(mousePointer.transform);
			bullet2.transform.Rotate(0,Random.Range(-7.5f,7.5f),0);
				AlertEnemies();
			break;
		}
